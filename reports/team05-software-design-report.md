...
### Composite Pattern
The *Composite pattern* allows clients to treat individual objects and compositions of objects uniformly by organizing them into tree structures. In Wireshark, this pattern is essential for representing a packet’s decoded data as a hierarchical tree. Network packets, indeed, are inherently nested (for example, an Ethernet frame contains an IP packet, which in turn contains a TCP segment and its associated fields). Without this pattern, the Wireshark engine would have to implement separate and complex logic to handle entire protocols versus individual fields, making navigation algorithms and GUI rendering extremely inefficient and rigid.

The *Component* role is fulfilled by the base structure proto_node (defined in epan/proto.h), which establishes the common interface and the pointers necessary for parent-child relationships between nodes. The role of *Composite* (the container) is implemented by `proto_tree` (a typedef of proto_node), which represents a protocol or a subtree capable of containing other nested elements. The *Leaf* role is represented by `proto_item`, which corresponds to the individual terminal fields of the protocol that cannot contain further children. The uniform interface allows dissectors to add elements recursively using functions such as `proto_tree_add_item()` in epan/proto.c, without worrying about whether they are adding data to the main tree or to a very deep subtree.

Instead of the Composite pattern, other solutions could include:
- *Iterator Pattern*: Accessing and navigating the various fields of the collection would be handled by a sequential iterator object, hiding the underlying data structure. It would completely decouple the UI display algorithms and search filters from the physical structure of the packet, allowing the internal structure to be changed (e.g., from a tree to a matrix) without impacting the rest of the software.
However, this is purely an access pattern, not a structuring one. On its own, it does not solve the problem of how to represent the hierarchy and nesting of protocols in memory, adding only a potentially redundant level of abstraction.
- *Flyweight Pattern*: the repetitive fields of a protocol would not be allocated from scratch each time for every packet, but would share a common “intrinsic state”, passing only the “extrinsic state” (the actual value read) dynamically. It would drastically reduce the high memory (RAM) consumption at runtime and the overhead of continuous allocation, which represent the main limitation of the current Composite implementation during prolonged captures. Still it would significantly increase architectural complexity in C to manage the clear separation of states. Furthermore, data access would become indirect, introducing a computational cost in terms of execution time that is incompatible with real-time requirements.

### Singleton Pattern
The *Singleton pattern* ensures that there is only one instance of a given data structure and provides a global access point to it. In Wireshark, this pattern is essential for managing dissector tables, which act as a “switchboard” to route packets to the correct protocol. Without this pattern, each module could create its own separate routing table, in this way the central engine would query one table to route the packet, while a protocol would register with another copy, causing the network analysis to fail irreversibly due to the lack of a Single Source of Truth.

Since Wireshark is written in C, the pattern is adapted to work around the lack of classes and private constructors. The *Singleton* role is implemented by hiding the data: a pointer to a global hash table (`GHashTable`) is declared within the epan/packet.c file and marked with the `static` keyword (e.g., `static GHashTable *dissector_tables = NULL;`). This makes it invisible to other files in the project, preventing accidental clones. The role of the *Global Access Point* (the equivalent of the classic `getInstance()` method) is performed by specific public functions such as `find_dissector_table()`. This interface forces all dissectors to call centralized functions, which operate safely and in a controlled manner on the single instance existing in memory.

Instead of the Singleton pattern, other solutions could include:

- *Proxy Pattern*: Access to the table instance would be mediated by a proxy object acting as an intermediary. This would allow the table’s memory allocation to be deferred until the moment of actual use (Virtual Proxy), or would enable the addition of control logic to prevent external plugins from overwriting fundamental routing rules (Protection Proxy). However, since routing in Wireshark occurs for every single captured packet, adding this control layer for every read operation would introduce a computational overhead that would slow down performance to a degree incompatible with real-time requirements.

- *Facade Pattern*: Instead of merely ensuring the uniqueness of a single data structure (the tables), one could create a global Facade that hides an entire complex group of interfaces and routing logic. This would allow individual dissectors to interact only with a unified, high-level interface, without being exposed to the underlying management details. Nevertheless, in a layered system like Wireshark, incorporating an entire functional area behind a single “Facade” would risk turning the latter into a massive bottleneck (a monolithic God Object). The current Singleton, on the other hand, is specifically limited to ensuring the uniqueness of the resource, keeping the architecture leaner and more modular.
